...
function PublicPlaylistResults({ q, onSelect }: { q: string; onSelect: (id: string) => void }) {
  const { data: results = [] } = useQuery({
    queryKey: ['social', 'search', q],
    queryFn: () => api.social.searchPublicPlaylists(q),
    enabled: q.length > 1,
    staleTime: 10_000,
  })
  if (!results.length) return null
  return (
    <div className="absolute top-full left-0 right-0 mt-1 bg-xkc-surface border border-xkc-border rounded-lg shadow-xl z-50 py-1 max-h-60 overflow-y-auto">
      <div className="px-3 py-1 text-[10px] text-xkc-muted uppercase tracking-wider border-b border-xkc-border mb-1">
        Public playlists
      </div>
      {results.map(pl => (
        <button key={pl.id}
          onClick={() => onSelect(pl.id)}
          className="w-full text-left px-3 py-2 hover:bg-xkc-border/50 flex items-center gap-2">
          <Globe size={12} className="text-xkc-accent flex-shrink-0" />
          <div className="min-w-0">
            <div className="text-xs text-xkc-text truncate">{pl.name}</div>
            <div className="text-[10px] text-xkc-muted">{pl.owner_username} · {pl.track_count} tracks</div>
          </div>
        </button>
      ))}
    </div>
      ...
      <div className="flex flex-col h-screen bg-xkc-bg">
        ...
        <header className="flex items-center gap-3 px-4 py-2 border-b border-xkc-border bg-xkc-surface flex-shrink-0 h-12">
          ...
          <div className="flex-1 max-w-md relative">
            ...
            <input
              type="text"
              placeholder="Search tracks or public playlists…"
              value={searchQuery}
            ...
            />
            {searchQuery && <PublicPlaylistResults q={searchQuery} onSelect={(id) => { setSelectedPlaylistId(id); setSearchQuery('') }} />}
          </div>
